...
## EQ recording and linked input filters

Reviewed the approximately 100-second EQ recording. The dedicated WING EQ view reproduces the wide logarithmic frequency graph, yellow response/fill, six bands, blue selection, gain/Q/frequency controls, low/high shelf-to-parametric switches and LC/HC controls. Band handles drag frequency and gain together; double-click resets gain. Untouched model, mix, freeze, filter-detail, reset and band-solo items remain fixed visuals.

Input and EQ use the same per-channel lowCut/highCut booleans and cutoff frequencies. Either page changes the same displayed response and mixer thumbnail; switching pages does not reset filter frequencies. Input filters remain independent from EQ-band bypass. Existing band frequencies, flat gains, EQ bypass and low-cut frequency defaults are preserved; high-cut frequency initializes to 11.91 kHz from the recording, with both cuts off. Low/high bands initialize as shelving. EQ curves are illustrative, with bounded band gains, Gaussian bell shapes, smooth shelves and fixed cut slopes; exact firmware response and unshown cutoff limits are not verified. No audio processing is introduced.

EQ editing now previews the configured band response even when EQ is bypassed, with an explicit settings-preview label. This prevents the initial bypass state from hiding hills and valleys during editing. The underlying effective-response function still respects bypass; the editor and mixer thumbnails explicitly request a settings preview. Band gain/Q/frequency and shared cuts remain unchanged.

## Compressor recording

Reviewed Comp.mov. The dedicated compressor uses the recorded dynamics header, blue transfer curve, draggable threshold, green attack/hold/release envelope and two rows of timing/threshold/ratio/knee controls. Gain (makeup) is in the header. Detector RMS, envelope LOG, auto-envelope OFF, mix 100%, key source SELF, solo, crossover FLAT and key filter FLAT remain fixed visuals because they were not demonstrated. Gate and compressor share a compact slider component while retaining independent state.

The transfer curve previews configured threshold/ratio/knee even while bypassed. Makeup gain is retained separately; the recorded transfer plot does not visibly shift when makeup gain changes. Existing compressor defaults are preserved, with knee initialized to 3. Envelope geometry, knee-width mapping and unshown ranges are illustrative, not verified firmware/DSP behavior. No live meters or audio processing are added.
